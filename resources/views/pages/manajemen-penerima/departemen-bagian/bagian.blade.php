                ...
                <form action="{{ route('manajemen-penerima.departemen-bagian.bagian.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label for="fg-01">Nama Bagian <span class="text-red">*</span></label>
                            <input type="text" class="form-control @error('nama', 'store') is-invalid @enderror"" id="fg-01" name="nama" value="{{ $errors->hasBag('store') ? old('nama') : '' }}" placeholder="Isikan nama bagian">
                            @error('nama', 'store')
                            ...
                            @foreach ($bagian as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-info btn-xs btn-menu" data-toggle="modal" data-target="#modal-detail" data-id="{{ $item->id_bagian }}"><i class="fas fa-eye"></i></button>
                                        <button type="button" class="btn btn-warning btn-xs btn-menu text-white" data-toggle="modal" data-target="#modal-ubah" data-id="{{ $item->id_bagian }}"><i class="fas fa-edit"></i></button>
                                        <button type="button" class="btn btn-danger btn-xs btn-menu" data-toggle="modal" data-target="#modal-hapus" data-id="{{ $item->id_bagian }}"><i class="fas fa-trash-alt"></i></button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body mt-3">
                    <ul class="list-group list-group-unbordered mb-3">
                        ...
                        <li class="list-group-item">
                            <strong class="text-muted"><i class="far fa-stop-circle mr-1"></i> Nama</strong>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" enctype="multipart/form-data">
                    @method('put')
                    @csrf
                    <input type="hidden" name="id_bagian" value="{{ $errors->hasBag('update') ? old('id_bagian') : '' }}">
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="fg-11">Nama Bagian <span class="text-red">*</span></label>
                            <input type="text" class="form-control @error('nama', 'update') is-invalid @enderror"" id="fg-11" name="nama" value="{{ $errors->hasBag('update') ? old('nama') : '' }}" placeholder="Isikan nama bagian">
                            @error('nama', 'update')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
